...
OPTIONS
       ...
       --names
           ...
           after its path, so that files are referred to by an identifier the
           compiler checks rather than by a string.

       --names-module=MODULE
           Name of the module generated by --names. Defaults to Name. Only
           meaningful together with --names.

       --names-strip-ext
           Drop the file extension when deriving identifiers for --names, so
           that some/secret.age becomes some__secret instead of
           some__secret_age. Only meaningful together with --names, and
           rejected if two files then map to the same identifier.
